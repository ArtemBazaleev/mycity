...
public final class NavUtils {
    ...
    private static boolean onNavDestinationSelected(
            @NonNull MenuItem item,
            @NonNull NavController controller
    ) {
        NavOptions.Builder builder = new NavOptions.Builder()
                .setLaunchSingleTop(true);
        if (controller.getCurrentDestination().getParent().findNode(item.getItemId())
        ...
        if ((item.getOrder() & Menu.CATEGORY_SECONDARY) == 0) {
            builder.setPopUpTo(findStartDestination(controller.getGraph()).getId(),
                    false);
        }
        NavOptions options = builder.build();
        try {
            controller.navigate(item.getItemId(), null, options);
            return true;
